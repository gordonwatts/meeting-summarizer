...
def test_store_and_resolve_api_key(workspace_tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cli_api_key_precedence(monkeypatch: pytest.MonkeyPatch, workspace_tmp_path) -> None:
    home = workspace_tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(config.Path, "home", lambda: home)
    (home / ".env").write_text(f"{config.API_KEY_ENV_VAR}=from-file\n", encoding="utf-8")
